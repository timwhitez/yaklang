...
func (s *Server) MITM(stream ypb.Yak_MITMServer) error {
	...
	if downstreamProxy != "" {
		...
		conn, err := netx.DialTimeout(5*time.Second, "tcp", utils.HostPort(host, port))
		if err != nil {
			feedbackToUser(fmt.Sprintf("下游代理检测失败 / downstream proxy failed:[%v] %v", downstreamProxy, "代理不通（Proxy Cannot be connected）"))
			if errors.Is(err, netx.ErrProxyAuthFail) {
				return utils.Errorf("proxy username or password wrong: %v", downstreamProxy)
			}
