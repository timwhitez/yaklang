...
func (b *FunctionBuilder) BuildSyntaxBlock(builder func()) {
	...
	if !ssautil.BuildSyntaxBlock(Scope, func(sub *ssautil.ScopedVersionedTable[*Variable]) bool {
...
func (i *IfBuilder) Build() {
	...
	for _, item := range i.items {
		...
		ScopeBuilder.BuildItem(
			// ifStmt := builder
			func(sub *ssautil.ScopedVersionedTable[*Variable]) {
	...
	if i.elseBody != nil {
		ScopeBuilder.BuildElse(func(sub *ssautil.ScopedVersionedTable[*Variable]) {
			SSABuilder.CurrentBlock.ScopeTable = sub
			i.elseBody()
		})
	}
	...
	if len(DoneBlock.Preds) != 0 {
		...
		ScopeBuilder.BuildFinish(generalPhi(i.builder))
	}
}
