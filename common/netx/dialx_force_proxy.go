...
var ErrProxyAuthFail = errors.New("proxy username or password wrong")
...
func isHTTPConnectWork(c net.Conn) error {
	...
	if code < 200 || code > 400 {
		if code == 407 {
			return utils.Error("username or password error")
		}
		return utils.Error("invalid status code")
	}
	for {
		...
		if ok {
			...
			case "transfer-encoding":
				return utils.Error("transfer-encoding should not be set")
			}
		}
